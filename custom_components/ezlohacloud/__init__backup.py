...
from .frp_helpers import install_frpc
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    # NOTE: Maybe get the frp version, repo link and from ezlo api instead of constants.
    build_arch = os.uname().machine
    frp_version = "0.61.0"
    try:
        await hass.async_add_executor_job(install_frpc, build_arch, frp_version)
    except Exception as e:
        _LOGGER.error(f"Failed to set up FRPC: {e}")  # noqa: G004
        return False
...
async def generate_config_file(
    ...
    """Generate FRPC configuration file using API endpoint."""
    try:
        # Fetch configuration from API
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://frp.ezlo.com/server/config", timeout=10
            ) as response:
        ...
        config_data = {
            ...
            "server": {
                "addr": api_config["serverAddr"],
                "port": api_config["serverPort"],
            },
            "webServer": {
                "port": config["web_server_port"],
                "user": config["web_server_user"],
                "password": config["web_server_password"],
            },
            "proxies": [],
        }

        # Add proxies from API response
        for proxy in api_config["proxies"]:
        ...
        _LOGGER.error("Failed to fetch API configuration: %s", err)
        raise
    except KeyError as err:
        _LOGGER.error("Missing required field in configuration: %s", err)
        raise
    except Exception as err:
        _LOGGER.error("Failed to generate configuration: %s", err)
        raise
